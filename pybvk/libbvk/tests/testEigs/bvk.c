...
//outputs bin centers
  ...
  fprintf(stderr,"DOS [%le,%le] res %le -> %d bins\n",wmin,wmax,wres,nbins);
  *wMin=wmin;
  *nBins=nbins;
  return bin;
}
...
int bvkCompute(System* system,int nq,QPoint* qs,
  ...
  *pws=eigValue=eigValues=(EigenValue*)malloc(sizeof(EigenValue)*nq*d.n);
  ...
  for(int qnum=0;qnum<nq;qnum++) {
    if(progress++%10000==0) fprintf(stderr,".");
    QPoint* q=qs+qnum;
    ...
    if(info!=0) { fprintf(stderr,"zheev failed\n"); abort(); }
...
//        memcpy(eigVector+=d.n,&d.e[d.n*i],sizeof(doublecomplex)*d.n);
    ...
    if(pes && eigenvectorsStdout) {
      // Write the eivenvalues and eigenvectors to stderr // stdout
      for(int i=0;i<d.n;i++) {
        fprintf(stderr,"%le ",v[i]);
        for(int j=0;j<d.n;j++)
          fprintf(stderr,"%7.4lf/%7.4lf ",d.e[d.n*i+j].real,d.e[d.n*i+j].imag);
            printf("\n");
      }
    }
