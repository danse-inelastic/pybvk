...
static int _computeBonds(System* system) {
  ...
  if(system->nBonds) {
    ...
    for(int n=0;n<c->sites;n++) {
      Site* site=&sites[n];
      fprintf(stderr,"Site %d: atom %d at ",n,site->atomType);
      vectorDump2stderr(&site->v);
      fprintf(stderr,"\n");
    }
    for(int b=0;b<c->cbonds;b++) {
      CanonicalBond* bond=&cbonds[b];
      fprintf(stderr,"CBond: %d->%d ",bond->fromAtomType,bond->toAtomType);
      vectorDump2stderr(&bond->v); fprintf(stderr," FCT:\n");
      matrixDump2stderr(&bond->fct);
    }
  }
    ...
    for(int b=0;b<c->cbonds;b++) {
      ...
      for(int s=0;s<c->symmetries;s++) {
        ...
        if(image.x+TOL<0 || image.x-TOL>1 ||
          ...
          fprintf(stderr,"CRAP! Shifted sbondv is not in cell.  In cell coords:\n");
          vectorDump2stderr(&image); fprintf(stderr,"\n");
          abort();
        }
        ...
        for(int s=0;s<c->sites;s++) {
          ...
          if(vectorSame(&site->v,&sbondv)) {
            if(toSite!=NULL) {
              fprintf(stderr,"CRAP: image maps to more than one site!\n");
              abort();
            }
...
// This isn't an error condition anymore.
          fprintf(stderr,"CRAP: image doesn't map to a site!\n");
          fprintf(stderr,"fromSite %d, bond %d and symmetry %d are:\n",fromSiteN,b,s);
          vectorDump2stderr(&bond->v); fprintf(stderr,"\n");
          matrixDump2stderr(&symmetry->m);
          fprintf(stderr,"image in cell (which is not a site) is:\n");
          vectorDump2stderr(&sbondv); fprintf(stderr,"\n");
          fprintf(stderr,"image in cell in cell coords is:\n");
          vectorDump2stderr(&image); fprintf(stderr,"\n");
          fprintf(stderr,"bond to (before shift to cell to become image) is:\n");
          vectorDump2stderr(&sbondvdebug); fprintf(stderr,"\n");
          abort();
#endif
...
//        if(fromSite-sites>toSite-sites) continue;
        ...
        if(bonds) {
          fprintf(stderr,"  bond from %s ",atoms[fromSite->atomType].comment);
          vectorDump2stderr(&fromSite->v);
          fprintf(stderr," to %s ",atoms[toSite->atomType].comment);
          Vector toV; memcpy(&toV,&fromSite->v,sizeof(toV));
          mvMultiply(&sbondv,&symmetry->m,&bond->v);
          vectorScaleAndAccumulate(&toV,&sbondv,1.0);
          vectorDump2stderr(&toV);
          fprintf(stderr,"\n");
...
int systemComputeBonds(System* system) {
  ...
  fprintf(stderr,"There are %d bonds\n",system->nBonds);
